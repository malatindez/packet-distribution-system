    ...
    public:
        ...
        DiffieHellmanHelper()
        {
            EVP_PKEY_WRAPPER params;
            {
                ...
                utils::AlwaysAssert(EVP_PKEY_paramgen_init(pkey_ctx_.get()) == 1,
                                    "Error generating DH parameters.");
                utils::AlwaysAssert(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(
                                        pkey_ctx_.get(), NID_X9_62_prime256v1) == 1,
                                    "Error generating DH parameters.");
                EVP_PKEY *params_val = nullptr;
                utils::AlwaysAssert(EVP_PKEY_paramgen(pkey_ctx_.get(), &params_val) == 1,
                                    "Error generating DH parameters.");
            ...
            utils::AlwaysAssert(EVP_PKEY_keygen_init(keygen_ctx_.get()) == 1,
                                "Error generating DH key");
            EVP_PKEY *pubkey = nullptr;
            utils::AlwaysAssert(EVP_PKEY_keygen(keygen_ctx_.get(), &pubkey) == 1,
                                "Error generating DH key");
        ...
        ByteArray get_public_key() const
        {
            ...
            utils::AlwaysAssert(PEM_write_bio_PUBKEY(bio.get(), pubkey_.get()) > 0,
        ...
        ByteArray get_shared_secret(const ByteView peer_key_bytes) const
        {
            ...
            utils::AlwaysAssert(EVP_PKEY_derive_init(ctx.get()) == 1,
                                "Error initializing PKEY context.");
            utils::AlwaysAssert(EVP_PKEY_derive_set_peer(ctx.get(), peer_key.get()) == 1,
                                "Error initializing PKEY context.");

            size_t len;
            utils::AlwaysAssert(EVP_PKEY_derive(ctx.get(), NULL, &len) == 1,
                                "Error initializing PKEY context.");
