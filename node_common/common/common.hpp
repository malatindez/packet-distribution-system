...
namespace node_system
{
     ...
     * 
     * This template function converts the first N bytes of the given ByteView to an integer value of the specified type,
     ...
     * @throws utils::AssertionError if the byte_view size is less than N bytes.
     */
    template <typename Integer>
    [[nodiscard]] inline Integer bytes_to_integer(const ByteView byte_view)
    {
        utils::Assert(byte_view.size() >= sizeof(Integer), "The byte array is too small to be converted to the specified integer type");
     ...
     * 
     * This template function converts an integer value of the specified type to a ByteArray of appropriate size,
    ...
    struct ByteArray : public std::vector<std::byte>
    {
        ...
        void append(const std::vector<T> &other)
        {
            ...
            {
                std::transform(other.begin(), other.end(), std::back_inserter(*this), [](const T &t)
         ...
         * 
         * @note Input integer should take into account endianness. Check out @bytes_to_uint32 and @uint32_to_bytes functions.
         * 
         * @note Other conversions are forbidden, because of alignment/endianness and compiler features on other systems/compilers. 
         * 
